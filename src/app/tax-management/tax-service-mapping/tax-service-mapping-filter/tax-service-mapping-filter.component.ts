...
@Component({
  selector: 'app-tax-service-mapping-filter',
  templateUrl: './tax-service-mapping-filter.component.html',
  styleUrls: ['./tax-service-mapping-filter.component.scss']
})
export class TaxServiceMappingFilterComponent implements OnInit, OnDestroy {
  ...
  merchantSelectionChange(merchantId): any {
    this.branchList = [];
    if (merchantId) {
      this.manageGridService.loadFilters$.next(`serviceMappingBean.branch.merchantBean.merchantId==${merchantId}`);
      const param = {
  ...
  branchSelectionChange(branch: any): any {
    if (branch) {
      ...
      this.manageGridService.loadFilters$.next(`serviceMappingBean.branch.branchId==${branch.branchId}`);
  ...
  serviceSelectionChange(service): any {
    ...
    if (service) {
      const filter = `serviceMappingBean.branch.branchId==${this.selectedBranch.branchId}##serviceMappingBean.services.serviceId==${service.serviceId}`;
