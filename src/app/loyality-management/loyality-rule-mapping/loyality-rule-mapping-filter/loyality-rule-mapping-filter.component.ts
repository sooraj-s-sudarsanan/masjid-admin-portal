...
@Component({
  selector: 'app-loyality-rule-mapping-filter',
  templateUrl: './loyality-rule-mapping-filter.component.html',
  styleUrls: ['./loyality-rule-mapping-filter.component.scss']
})
export class LoyalityRuleMappingFilterComponent implements OnInit, OnDestroy {
  ...
  serviceSelectionChange(service): any {
    ...
    if (service) {
      const filter = `branchId==${this.selectedBranch.branchId}##serviceId==${service.serviceId}`;
